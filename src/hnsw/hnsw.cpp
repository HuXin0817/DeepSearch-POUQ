//
// Created by Koschei on 2025/5/10.
//
...
Hnsw::Hnsw(int dim, const std::string& metric, int R, int L)
    : nb(), dim(dim), M(R / 2), efConstruction(L), final_graph() {
...
int Hnsw::Dim() { return dim; }

void Hnsw::Build(float* data, int N) {
  nb = N;
